...
internal interface ISettingApi
{
	...
	[Get("/api/apps/{id}/setting/{environment}/item")]
	Task<IApiResponse<string>> GetItemsAsync(long id, string environment, [Header("x-format")] string type, CancellationToken cancellationToken = default);
	...
	Task<IApiResponse> CreateAsync(long id, string environment, [Body] SettingCreateDto data, CancellationToken cancellationToken = default);
